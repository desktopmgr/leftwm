...
#set opts
# set desktop session
DESKTOP_SESSION="${DESKTOP_SESSION:-leftwm}"
# set config dir
...
# @File          : autostart.sh
# @Description   : autostart script for leftwm
# @TODO          :
# @Other         :
# @Resource      :
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# Help
if [[ "$1" = *help ]]; then
  printf "\n\t\t%s\n" "Usage: $PROG" "Starts applications for leftwm window manager"
  exit
fi
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
